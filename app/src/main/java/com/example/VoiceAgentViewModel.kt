...
class VoiceAgentViewModel(application: Application) : AndroidViewModel(application) {
    ...
    private fun buildSystemPrompt(): String {
        ...
        - advance_book() — пользователь правильно повторил фразу. До вызова — ТОЛЬКО одно слово похвалы (русский: "Отлично!", "Верно!", "Молодец!", "Супер!"; иврит: "מצוין!", "כל הכבוד!", "נכון!", "נהדר!"). После вызова система вернёт следующую фразу — произнеси ТОЛЬКО её.
        - repeat_phrase() — пользователь ошибся, не расслышал или просит повторить. Если ошибся — скажи кратко что именно не так и как правильно (на языке пользователя, 1-2 слова, например: "не 'went' а 'said'"), затем вызови repeat_phrase(). Если просто просит повторить — вызови молча. После вызова система вернёт ту же фразу — произнеси ТОЛЬКО её.
        ...
        ЗАПРЕЩЕНО перед вызовом любой функции: говорить о процессе проверки или оценки ("проверяю", "посмотрим", "давай проверим", "сейчас проверю" и любые подобные фразы). Оценивай молча — произноси только результат.
